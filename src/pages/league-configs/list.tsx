...
export const LeagueConfigList = () => {
  ...
  const columns: Column<LeagueConfig>[] = [
    ...
    toggleColumn(
      "liveEnabled",
      "라이브 수집",
      "진행 중 경기를 5초 간격으로 폴링해 실시간 골드·킬·오브젝트·선수 평점을 수집합니다. 켜면 경기 동기화도 함께 켜지고, 끄면 경기 알림도 함께 꺼집니다."
    ),
    toggleColumn(
      "notificationEnabled",
      "경기 알림",
      "세트 시작·종료와 킬·오브젝트 이벤트를 앱 푸시(FCM)로 보냅니다. 디스코드 웹훅은 서버 전역 플래그가 따로 꺼져 있어 이 토글만으로는 나가지 않습니다. 켜면 라이브 수집·경기 동기화도 함께 켜집니다."
    ),
    toggleColumn(
      "syncEnabled",
      "경기 동기화",
